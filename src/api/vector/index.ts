...
export default function(app: Express, impl: t.VectorApi) {
	app.post(
		'/api/v1/vector/query',
			...
			try {
				function __body() {
					...
					if (__mimeType === 'application/json') {
						return v.modelApiVectorVectorRequestFromRequest('body', req.body)
					}
					console.error(`Invalid request content type: ${__contentType}`)
					throw new Error(`Invalid request content type: ${__contentType}`)
				}

				impl.vectorQuery(__body()).then(function (response) {
					if (response.status === 200) {
						let body: any
						try {
							body = v.modelApiVectorVectorResponseToResponse('response', response.body)
						} catch (error) {
							console.error('Invalid response body in vector.vectorQuery', error)
							res.status(500)
							res.send()
							return
						}
					...
					console.error('Unexpected error in vector.vectorQuery', error.stack || error)
					res.status(500)
					res.send()
				})
			} catch (error) {
