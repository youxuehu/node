...
export default function(app: Express, impl: t.ContentApi) {
	app.post(
			...
			try {
				function __body() {
					...
					if (__mimeType === 'application/json') {
						return v.modelApiContentValidateRequestFromRequest('body', req.body)
					}
					console.error(`Invalid request content type: ${__contentType}`)
					throw new Error(`Invalid request content type: ${__contentType}`)
				}

				impl.contentValidate(__body()).then(function (response) {
					if (response.status === 200) {
						let body: any
						try {
							body = v.modelApiContentValidateResponseToResponse('response', response.body)
						} catch (error) {
							console.error('Invalid response body in content.contentValidate', error)
							res.status(500)
							res.send()
							return
						}
					...
					console.error('Unexpected error in content.contentValidate', error.stack || error)
					res.status(500)
					res.send()
				})
			} catch (error) {
